...
function validatePhase1Internal_() {
  ...
  var scheduledTriggers = ScriptApp.getProjectTriggers().filter(function (trigger) { return trigger.getHandlerFunction() === 'scheduledDailyJob'; });
...
function onOpen() {
  ...
  try {
    if (cleanText_(settingsMapFromSpreadsheetV84_(spreadsheet).FILE_ROLE) === V84_BACKUP.FILE_ROLE_BACKUP) {
      ui.createMenu('資產記錄備份（唯讀）')
        .addItem('查看唯讀說明', 'showBackupReadOnlyNoticeV84_')
        .addToUi();
      spreadsheet.toast('這是封存備份，只供查看與還原來源使用。請回到目前正式的「資產記錄」操作。', '資產記錄備份', 10);
      return;
    }
  } catch (ignore) {}
  ui.createMenu('資產記錄 V8.4.0')
    ...
    .addItem('重算持倉與績效', 'rebuildAllPerformance')
    .addItem('完整更新', 'refreshAllCurrentData')
    .addSeparator()
    .addItem('從 2024 年開始重建趨勢', 'startTrendRebuildFrom2024')
    .addItem('續跑趨勢重建', 'continueTrendRebuild')
    .addItem('補建缺漏快照', 'rebuildMissingTrendSnapshots')
    .addItem('更新 Dashboard', 'refreshDashboard')
    .addSeparator()
    .addItem('驗證第一階段', 'validatePhase1')
    .addItem('驗證第二階段', 'validatePhase2')
    .addItem('V8.3 正式清理前驗證', 'validatePhase3Preflight')
    .addItem('驗證第三階段', 'validatePhase3')
    .addItem('驗證 V8.3.1 績效與 API', 'validateV831PerformanceAndApi')
    .addToUi();
}

function showBackupReadOnlyNoticeV84_() {
  SpreadsheetApp.getUi().alert('這是封存備份，只供查看與還原來源使用。請回到目前正式的「資產記錄」操作。');
}
