...
function installV82() {
  if (/^8\.[34]\./.test(V81.VERSION) && typeof installV831 === 'function') return installV831();
  try {
    return withDocumentLock_(function () {
...
function scheduledDailyJob() {
  try {
    ...
    return withDocumentLock_(function () {
      ...
      if (typeof setJobStateV83_ === 'function') {
        setJobStateV83_('market', 'running', { startedAt: jobStartedAt, finishedAt: '', error: '' });
        setJobStateV83_('rebuild', 'running', { startedAt: jobStartedAt, finishedAt: '', error: '' });
      }
      ensureV81Schema_();
      ...
      if (typeof setJobStateV83_ === 'function') {
        ...
        setJobStateV83_('market', fx.failed === 0 && prices.failed === 0 ? 'success' : 'partial', { finishedAt: jobFinishedAt, error: '' });
        setJobStateV83_('rebuild', rebuild.errorCount === 0 ? 'success' : 'partial', { finishedAt: jobFinishedAt, error: '' });
      }
    ...
    // 排程撞上備份／還原鎖或誤在封存副本執行時必須零寫入略過。
    if (error && (error.apiCode === 'SYSTEM_BUSY' || error.apiCode === 'NOT_PRIMARY_FILE')) {
      return apiResult_(false, error.apiCode, error.message, { skipped: true });
    }
    if (typeof setJobStateV83_ === 'function') {
      var jobFailedAt = nowSheet_();
      setJobStateV83_('market', 'error', { finishedAt: jobFailedAt, error: error.message });
      setJobStateV83_('rebuild', 'error', { finishedAt: jobFailedAt, error: error.message });
    }
