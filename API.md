...
既有 v8.3 資料 action 名稱維持相容，但所有受保護 action 都只接受 Session。前端只使用上述 v8.5 名稱。`snapshots.rebuildAll`、還原、變更密碼及登出所有裝置都需符合 action scope 的 Elevated Token。
...
常用登入錯誤碼：`AUTH_REQUIRED`, `AUTH_NOT_CONFIGURED`, `AUTH_INVALID_CREDENTIALS`, `AUTH_LOCKED`, `AUTH_SESSION_EXPIRED`, `AUTH_SESSION_REVOKED`, `AUTH_ELEVATION_REQUIRED`, `AUTH_ELEVATION_EXPIRED`。
